        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            <Link
              ...
              title={itemCount > 0 ? `${itemCount} ${itemCount === 1 ? 'item' : 'items'} in cart` : 'Cart is empty'}
            >
              {/* Shopping basket SVG */}
              <svg
                ...
                stroke="currentColor"
              >
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9h18M9 9V6a3 3 0 016 0v3M5 9l1 11h12l1-11H5z" />
              </svg>
              {/* Colored dot indicator */}
              {itemCount > 0 && (
                <span className="absolute top-1 right-1 block h-2.5 w-2.5 rounded-full bg-primary" aria-hidden="true" />
              )}
            </Link>
            <button
              onClick={toggleTheme}
